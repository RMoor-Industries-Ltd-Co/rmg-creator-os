...
| `FOUNDER_PRINCIPALS` | master-atelier prd | **Needs adding** — B1.2 §13 step 4 (2026-09-11). `email=principalId` pairs (e.g. `rahm@rmasters.group=rahm@business`), comma-separated for more than one. Distinct from `AUTH_ALLOWED_EMAILS` — unset means no founder (every founder-gated write refuses), never "everyone". Not yet consumed by a live write path (§13 step 6 is blocked on the A-Roll pin), so this can be added at any time before step 6 ships without changing current behavior. |
