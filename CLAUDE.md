...
| `STEP_UP_COOKIE_SECRET` | master-atelier prd | **Resolved** (2026-09-12). Live in production (`readiness.step_up: enabled`). The GCP console redirect URI (`https://rmg-creator-os.rmasters.group/stepup-callback.html` on the `GOOGLE_CLIENT_ID` OAuth client) has also been added. Interactive end-to-end validation is deferred until the first Founder-gated write path exists to trigger it — see `docs/atelier/b1-3-operational-activation.md`. |
| `FOUNDER_PRINCIPALS` | master-atelier prd | **Resolved** (2026-09-12). Corrected to `email=principalId` syntax, all three of the founder's emails mapping to `rahm@business`; live in production (`readiness.founder_approval: enabled`). See `docs/atelier/b1-3-operational-activation.md` and `docs/atelier/audits/incident-2026-09-11-founder-principal-count-object-keys.md` (the `disabled_no_principals` readiness report during the interim was a reporting defect, not a Doppler or parser problem). |
